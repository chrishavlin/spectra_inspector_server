...
def test_get_single_channel_image(edax_path_handler):
    ...
    im = get_single_channel_image(edax_path_handler, fake_filename, 2)
    ...
    im = get_single_channel_image(
        edax_path_handler, fake_filename, 2, index0_range=(0, 4)
    )
    new_expected = (4, expected_shp[1])
    assert im.shape == new_expected

    im = get_single_channel_image(
        edax_path_handler, fake_filename, 2, index1_range=(0, 5)
    )
